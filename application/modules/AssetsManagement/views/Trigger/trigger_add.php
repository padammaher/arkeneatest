...
foreach ($trigger_edit_list as $trigger_edit_data) {
                    ...
                    <form class="form-horizontal form-label-left" action="<?php echo base_url() ?>trigger_add" method="POST">
                        ...
                        <div class="item form-group">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12"><input type="checkbox"  onclick="undisable(this);" name="check_email" id="check_email" <?php echo (set_value('check_email'))== 'on'? 'checked':''; ?> > Email</label>
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <?php ?>
                                <input type="text" name="email" id="email" value="<?php echo set_value('email',$trigger_edit_data['email']);?>" class="form-control" placeholder="abc@xyz.com" <?php if(!empty(set_value('check_email'))){ echo (set_value('check_email'))== 'on'? '':'readonly';} else{ echo 'readonly';} ?> >
                            </div>
                        </div>

                        <div class="item form-group">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12"><input type="checkbox"  onclick="undisable1(this)" name="check_contact" id="check_contact" <?php echo (set_value('check_contact'))== 'on'? 'checked':''; ?>> SMS</label>
                            <div class="col-md-6 col-sm-6 col-xs-12">
