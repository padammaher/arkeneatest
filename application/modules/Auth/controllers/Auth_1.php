...
class Auth extends MY_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model(array('users', 'group_model','menu'));
    ...
    public function users() {
        ...
        if (!$this->ion_auth->logged_in()) {
            ...
            //list the users
            $user_data=$this->data['users'] = $this->ion_auth->users()->result();
            foreach ($this->data['users'] as $k => $user) {
                $this->data['users'][$k]->groups = $this->ion_auth->get_users_groups($user->id)->result();
            }
    ...
    public function login() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
                ...
                $user_roles=$this->group_model->getroles($current_user_id);
      
       
                if (isset($user_roles)) {
                     foreach($user_roles as $objData){
           
      
                    $group_name = $objData['role']['name'];
//                    var_dump($group_name);die;
                    if ($this->ion_auth->in_group($group_name)) {
                        if ($group_name == 'admin')
                            redirect('/', 'refresh');
                        else if ($group_name == 'members') {
                            redirect( '/', 'refresh');
                        } else if ($group_name == 'manager') {
//                            echo 'here';die;
                            redirect('dashboard', 'refresh');
                        } else if ($group_name == 'employee') {
//                            echo 'here';die;
                            redirect('dashboard', 'refresh');
                        }
                    } else {
